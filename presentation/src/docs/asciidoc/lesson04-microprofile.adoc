...
4. *Metrics*:
Benchmarks auf Fach- und Anwendungsebene
5. *OpenAPI*:
...
5. `src/main/resorces/application.property`
...
1. Im Ordner `lesson04-check/lesson04` befindet sich bereits ein Skelett. Es entspricht der Beispiellösung aus Aufgabe 3.4.
2. Eine Beispiellösung finden Sie im Ordner `solutions/lesson04-check/lesson04`
...
1. Im Ordner `lesson04-restlclient/lesson04` befindet sich bereits ein Skelett, jedoch nur für einen Service
2. Eine Beispiellösung finden Sie im Ordner `solutions/lesson04-restclient/`. Das Beispiel enthält beide Projekte
...
1. Der Eintrag *`quarkus.http.port=8080`* in der Datei `application.properties` bestimmt den HTTP-Port ‒
*`mvn compile quarkus:dev -Ddebug=5006`* den Debug-Port.
1. Im Ordner `lesson04-tracing/lesson04` befindet sich bereits ein Skelett, basierend auf der vorherigen Aufgabe.
Es enthält den Franchise- und den Order-Service.
2. Eine Beispiellösung finden Sie im Ordner `solutions/lesson04-tracing/`.
3. Integration von Jaeger in Elastic-Search: https://logz.io/blog/jaeger-and-the-elk-stack/
