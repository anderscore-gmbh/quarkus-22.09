...
* Benutzer mit Rolle `user` dürfen Bestellungen erstellen
* Für alle anderen Operationen ist die Rolle `admin` erforderlich
* Nutzen Sie Elytron. Legen Sie Kennwörter in der .properties Datei ab.
* Verschlüsseln Sie Zugriffe mit TLS

*Hinweise*

* Im Ordner `lesson05-security` ist ein Skelet. Es basiert auf der Beispiellösung von Aufgabe 3.1
* Das Archiv `certs.zip` in `lesson05-security` enthält TLS-Zertifikate. Sie gelten für
